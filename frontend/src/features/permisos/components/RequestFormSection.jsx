import { getToday, calcularFechaRegreso, contarDiasHabilesEnRango } from '../utils/requestHelpers';
          ...
          <div className="permisos-vacation-split">
            <div className="permisos-form-row">
              <div className="permisos-form-group">
                ...
                />
                <span className="permisos-field-help">Dias habiles segun jornada ({jornadaLaboral === 'LUNES_SABADO' ? 'L-S' : 'L-V'})</span>
              </div>
            ...
            {(() => {
              ...
              const tieneBloqueMayor = requests.some(r =>
                r.estado === 'APROBADA' &&
                new Date(r.fecha_inicio).getFullYear() === currentPeriod &&
                Number(r.dias_disfrutar || 0) >= 6
              );
              const minRestDays = tieneBloqueMayor ? 0 : 6;
              const fechaRegreso = formData.fecha_fin
                ? calcularFechaRegreso(formData.fecha_fin, 1, jornadaLaboral)
                : null;
              const breakdown = formData.fecha_inicio && formData.fecha_fin
                ? contarDiasHabilesEnRango(formData.fecha_inicio, formData.fecha_fin, jornadaLaboral)
                : null;

              return (
                <div className="permisos-vacation-summary">
                  <div className="permisos-summary-item">
                    <span>Dias calendario solicitados:</span>
                    <strong>{breakdown ? breakdown.calendario : '---'}</strong>
                  </div>
                  <div className="permisos-summary-item">
                    <span>Dias habiles descontados:</span>
                    <strong>{dDisfrutar}</strong>
                  </div>
                  <div className="permisos-summary-item">
                    <span>Dias no habiles omitidos:</span>
                    <strong>{breakdown ? breakdown.noHabiles : '---'}</strong>
                  </div>
                  <div className="permisos-summary-item">
                    <span>Dias en dinero:</span>
                    <strong>{dDinero}</strong>
                  </div>
                  <div className="permisos-summary-item">
                    <span>Total a descontar del saldo:</span>
                    <strong>{total}</strong>
                  </div>
                  <div className="permisos-summary-item">
                    <span>Saldo restante estimado:</span>
                    <strong>{Math.max(0, 15 - total)}</strong>
                  </div>
                  {fechaRegreso && (
                    <div className="permisos-summary-item">
                      <span>Fecha estimada de regreso:</span>
                      <strong>{fechaRegreso.toLocaleDateString('es-CO', { year: 'numeric', month: 'short', day: 'numeric' })}</strong>
                    </div>
                  )}
                  {dDisfrutar > 0 && dDisfrutar < minRestDays && (
                    <p className="permisos-validation-error">
                      Debes disfrutar al menos {minRestDays} dias de descanso en tu primer bloque del periodo.
                    </p>
                  )}
                  {total > 15 && (
                    <p className="permisos-validation-error">El total no puede exceder los 15 dias habiles.</p>
                  )}
                  {dDinero > 7 && (
